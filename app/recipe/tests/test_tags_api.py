...
class PrivateTagsApiTests(TestCase):
    ...
    def test_delete_tag_other_users_error(self):
        new_user = create_user(
            email='anotheruser@example.com', password='anotherpass123'
        )
        tag = Tag.objects.create(user=new_user, name='Appetizer')
    ...
    def test_filtered_tags_unique(self):
        tag = Tag.objects.create(user=self.user, name='Whatever Tag')
        Tag.objects.create(user=self.user, name='Tag Whatever')
        recipe1 = Recipe.objects.create(
            title='Another Test Recipe Again and Again',
            time_minutes=5,
            price=Decimal('1.99'),
            user=self.user,
        )
        recipe2 = Recipe.objects.create(
            title='Yet Another Test Recipe Again',
            time_minutes=55,
            price=Decimal('21.99'),
            user=self.user,
        )
